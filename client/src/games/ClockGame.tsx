...
const ROUTINE = [
  { h: 8, m: 0, order: 8 * 60, label: "起床", emoji: "🌅" },
  { h: 9, m: 0, order: 9 * 60, label: "上學", emoji: "🎒" },
  { h: 12, m: 0, order: 12 * 60, label: "放學", emoji: "🏫" },
  { h: 12, m: 30, order: 12 * 60 + 30, label: "午飯", emoji: "🍚" },
  { h: 2, m: 0, order: 14 * 60, label: "上家教課", emoji: "📚" },
  { h: 6, m: 30, order: 18 * 60 + 30, label: "吃晚餐", emoji: "🍲" },
  { h: 8, m: 30, order: 20 * 60 + 30, label: "洗澡", emoji: "🛁" },
  { h: 9, m: 0, order: 21 * 60, label: "睡覺", emoji: "🌙" },
];
...
export default function ClockGame(){
  ...
  const addRoutine=(item:typeof ROUTINE[number])=>{const expected=[...ROUTINE].sort((a,b)=>a.order-b.order)[routineOrder.length];if(item.label===expected.label){setRoutineOrder(c=>[...c,item]);setRoutinePool(c=>c.filter(x=>x.label!==item.label));playCorrect();}else{playWrong();speak("想想一天里先做什么，再做什么", "zh");}};
  ...
  return <div className="game-body clock-game-body"><div className="clock-modes">{modes.map(([key,label,Icon],i)=><button key={key} type="button" className={mode===key?"is-on":""} onClick={()=>setMode(key)}><em>{i+1}</em><Icon size={18}/>{label}</button>)}</div>
    ...
    {mode==="face"&&<section className="clock-panel"><header><span>關卡 1</span><h2>认识黄色小闹钟</h2><p>点短针、长针，再打开分钟外圈。</p></header><div className="clock-learn"><ClockFace time={faceTime} showMinuteRing={showRing} activeHand={activeHand} onHand={hand=>{setActiveHand(hand);speak(hand==="hour"?"我是时针，短短的，走得慢":"我是分针，长长的，走得快","zh");}}/><div className="clock-parts"><button type="button" className={activeHand==="hour"?"is-on":""} onClick={()=>{setActiveHand("hour");speak("我是时针，短短的，走得慢","zh");}}>短針＝時針</button><button type="button" className={activeHand==="minute"?"is-on":""} onClick={()=>{setActiveHand("minute");speak("我是分针，长长的，走得快","zh");}}>長針＝分針</button><button type="button" className={showRing?"is-on":""} onClick={()=>setShowRing(v=>!v)}>分鐘外圈 {showRing?"已打開":""}</button><div><b>分针指到 4</b><strong>4 × 5 = 20 分</strong><p>“一个字”也是 5 分钟，所以 4 个字就是 20 分钟。</p></div></div></div></section>}
    {mode==="read"&&<section className="clock-panel"><header><span>關卡 2 · 生活情境</span><h2>現在幾點？</h2><p>{CONTEXTS[Math.floor((readTime.h%12)/3)]??"看看时钟"}</p></header><div className="clock-question"><ClockFace time={readTime}/><div className="clock-options">{readChoices.map(v=><button key={v} type="button" className={readPick?v===format(readTime)?"is-right":v===readPick?"is-wrong":"":""} onClick={()=>answerRead(v)}>{v}</button>)}{readPick&&<button type="button" className="clock-next" onClick={()=>{setReadTime(randomTime(difficulty));setReadPick(null);}}>下一題 →</button>}</div></div></section>}
    {mode==="set"&&<section className="clock-panel"><header><span>關卡 3 · 最关键的输出</span><h2>请拨到 {format(target)}</h2><p>拖动两根针。分针走的时候，时针也会慢慢往前。</p></header><div className="clock-question"><ClockFace time={setTime} interactive onChange={setSetTime}/><div className="clock-set-card"><b>目标 {format(target)}</b><span>你拨的是 {format(setTime)}</span><button type="button" onClick={checkSet}><Check size={17}/> 检查</button><button type="button" onClick={()=>{const next=randomTime(difficulty);setTarget(next);setSetTime({h:next.h,m:0});setSetMessage(null);}}>换一个时间</button>{setMessage&&<p>{setMessage}</p>}</div></div></section>}
    {mode==="day"&&<section className="clock-panel"><header><span>關卡 4</span><h2>天天的一天</h2><p>按一天发生的先后顺序，把生活卡排进时间轴。</p></header><div className="clock-day-line">{routineOrder.map(item=><div key={item.label}><span>{item.emoji}</span><b>{item.label}</b><i>{format(item)}</i></div>)}</div><div className="clock-routine-pool">{routinePool.map(item=><button key={item.label} type="button" onClick={()=>addRoutine(item)}><span>{item.emoji}</span><b>{item.label}</b><i>{format(item)}</i></button>)}</div>{routinePool.length===0&&<div className="clock-success"><Check/> 一天排好了！时间让生活变得有顺序。</div>}</section>}
    {mode==="challenge"&&<section className="clock-panel"><header><span>關卡 5 · 可选速度挑战</span><h2>看谁最快读对</h2><p>平时不开计时；想挑战时再打开，不让速度盖过理解。</p></header><label className="clock-timer-toggle"><input type="checkbox" checked={timed} onChange={e=>{setTimed(e.target.checked);setTimeLeft(60);setChallengeScore(0);setChallengeRound(0);}}/> 开启 60 秒挑战</label><div className="clock-question"><ClockFace time={challengeTime}/><div className="clock-options"><strong><Star fill="#ffb400"/> {challengeScore} 分 · 第 {challengeRound+1} 题 {timed?`· ${timeLeft} 秒`:""}</strong>{timed&&timeLeft===0&&<p>時間到！你答對了 {challengeScore} 題。</p>}{challengeOptions.map(v=><button key={v} type="button" disabled={timed&&timeLeft===0} onClick={()=>answerChallenge(v)}>{v}</button>)}</div></div></section>}
  </div>;
}
