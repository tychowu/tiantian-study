...
function ChineseSpeakingGame() {
  ...
  const filtered = STORIES.filter(s => s.format === format && `${s.title} ${s.focus} ${s.alt}`.includes(query.trim()));
  const pageCount = Math.max(1, Math.ceil(filtered.length / 12));
  const visibleStories = filtered.slice(page * 12, (page + 1) * 12);
  useEffect(() => setPage(0), [format, query]);
  ...
  return <div className="game-body story-game">{story ? <StorySession key={story.id} story={story} onBack={() => setSelected(null)} onComplete={complete}/> : <><div className="story-picker-heading"><h1>挑一幅圖，說你的故事</h1><Voice text="挑一幅喜歡的圖。先看一看，再想一想，把事情接起來，說出你的故事。沒有倒數，也沒有唯一答案。" label="怎樣玩"/></div><p className="story-picker-intro">👀 找線索 → 💭 說想法 → 🚂 排先後 → 💬 講故事</p><nav className="story-format-tabs" aria-label="圖片類型"><button aria-pressed={format === "single"} onClick={() => setFormat("single")}>🖼️ 單頁看圖</button><button aria-pressed={format === "four"} onClick={() => setFormat("four")}>▦ 一頁四格</button></nav><p className="story-format-hint">{format === "single" ? "一幅大圖：找人物、動作、表情和位置。" : "四格連續圖：按順序觀察，說清楚事情怎樣發生。"}</p><label className="chinese-story-search">🔎 找一個故事<input type="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="輸入故事名稱或線索" /></label><p className="story-format-hint" role="status">找到 {filtered.length} 個故事 · 第 {page + 1} / {pageCount} 頁</p><div className="story-selection">{visibleStories.map((s, i) => <button key={s.id} onClick={() => setSelected(s.id)}><img src={`/images/stories/${s.image}.webp`} alt={s.alt} loading="lazy" draggable={false}/><span className="story-selection-copy"><small>故事 {page * 12 + i + 1} · {s.focus}</small><strong>{s.emoji} {s.title}</strong><span>{completed.includes(s.id) ? "🌟 已留下故事足印 · 再說新版本" : "打開圖片，一起說說看 →"}</span></span></button>)}</div>{filtered.length === 0 && <p>暫時找不到，試試其他字詞吧。</p>}{pageCount > 1 && <nav className="chinese-story-pages" aria-label="故事分頁"><button disabled={page === 0} onClick={() => setPage(p => p - 1)}><ArrowLeft size={17}/> 上一頁</button><span>{page + 1} / {pageCount}</span><button disabled={page + 1 >= pageCount} onClick={() => setPage(p => p + 1)}>下一頁 <ArrowRight size={17}/></button></nav>}<p className="story-picker-intro">可以說一句，也可以說一整段。你的想法，和別人不同也沒關係。</p></>}</div>;
}
